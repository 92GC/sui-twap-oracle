module futarchy::oracle {
    ...
    use std::debug;
    // ======== Safe Usage =========
    // Max twap accumualt is U256 Max ≈1.16 x 10^77
    // Worst-case TWAP daily accumulation:
    // Scale factor (10,000) * Milliseconds a day (24×3,600×1,000) * max price observation (?)

    // ===== Future Changes To Consider =====
    // Update once a window instead of every millisecond?
    // Let TWAP to fair market price during TWAP delay period and then reset accumulation when delay period ends.
   ...
   // ======== Testing Helpers ========
    ...
    #[test_only]
    public fun debug_get_state(oracle: &Oracle): (u64, u64, u256) {
    ...
    #[test_only]
    public fun debug_get_window_twap(oracle: &Oracle): u64 {
